...
// There are rp2040.fifo.X commands but i found them to be unreliable
int dumbmutex = 0;
...
void loop() {
  ...
  while(dumbmutex != CORE1_RENDER_SIGNAL);
...
void loop1() {
  ...
  while(dumbmutex != CORE0_RENDER_SIGNAL);

  // Render the second half -- render from 60 to 119 on core 1.
  amy.render(60,  AMY_OSCS, 1); 

  // Tell the other core we're done
  dumbmutex = CORE1_RENDER_SIGNAL;
}
