...
`tag` is optional. If you give one, you can replace or cancel that event later by resending `ticks="0,0,tag"` with the same `tag`. If you omit `tag` (a 1- or 2-value `ticks=`), the event is still scheduled and still fires, but it isn't addressable by any tag -- there's no way to cancel or replace it individually, so only omit `tag` for events you don't need to manage later.
