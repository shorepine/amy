...
| `iG`   | `note_output` | `note_output` | str | Where this synth's NOTE EVENTS go instead of to its oscillators: `'CV_GATE,<pitch>,<gate>[,<vel>[,<scale>[,<offset>[,<gate_volts>]]]]'`, `'MIDI_OUT,<channel 1..16>[,<forward_midi_in>]'`, or `'OFF'`. Deliberately the mirror of `ig` — same 1V/oct scale and offset, inverted — so a loopback patch round-trips a note unchanged. Such a synth costs no voices and no oscillators. See [note_outputs.md](note_outputs.md). |
