...
| `if`   | `synth_flags` | `synth_flags` | uint | Flags for synth creation, OR together: 1 = route note-ons/offs through the MIDI note map (`SYNTH_FLAGS_NOTES_VIA_MIDI`; used with the GM drum kit `patch=258` so note numbers select drum sounds); 2 = drop note-off events (`SYNTH_FLAGS_IGNORE_NOTE_OFFS`, for drums with long decays); 4 = negate pedal (`SYNTH_FLAGS_NEGATE_PEDAL`). |
