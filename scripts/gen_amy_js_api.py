...
def extract_coef_aliases(source: str):
    """Parse the dict_aliases map from parse_ctrl_coefs (old coef names)."""
    match = re.search(r"dict_aliases\s*=\s*(\{[^}]*\})", source)
    if not match:
        raise RuntimeError("Could not find dict_aliases in AMY __init__.py")
    return ast.literal_eval(match.group(1))
...
def generate_js(kw_map_list, coef_fields, coef_aliases, constants=None):
    ...
    coef_aliases_js = ("{" + ", ".join(f'{k}: "{v}"' for k, v in coef_aliases.items()) + "}") if coef_aliases else "{}"
    ...
    var AMY_COEF_ALIASES = %COEF_ALIASES%;
    ...
    js = js.replace("%COEF_ALIASES%", coef_aliases_js)
...
def main():
    ...
    coef_aliases = extract_coef_aliases(source)
    constants = extract_constants(AMY_CONSTANTS.read_text())
    js = generate_js(kw_map_list, coef_fields, coef_aliases, constants)
    if opts.check:
