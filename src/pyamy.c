...
static int parse_live_kwarg(amy_config_t *cfg, const char *key, PyObject *value) {
    ...
    if (strcmp(key, "chorus") == 0) {
        ...
        return 0;
    } else if (strcmp(key, "max_patterns") == 0) {
        llv = PyLong_AsLongLong(value);
        if (PyErr_Occurred()) return -1;
        if (llv < 0 || (unsigned long long)llv > UINT32_MAX) {
            PyErr_SetString(PyExc_ValueError, "max_patterns must be in range [0, 4294967295]");
            return -1;
        }
        cfg->max_patterns = (uint32_t)llv;
        return 0;
    } else if (strcmp(key, "max_pattern_tags") == 0) {
        llv = PyLong_AsLongLong(value);
        if (PyErr_Occurred()) return -1;
        if (llv < 0 || (unsigned long long)llv > UINT32_MAX) {
            PyErr_SetString(PyExc_ValueError, "max_pattern_tags must be in range [0, 4294967295]");
            return -1;
        }
        cfg->max_pattern_tags = (uint32_t)llv;
        return 0;
    } else if (strcmp(key, "max_pattern_instances") == 0) {
        llv = PyLong_AsLongLong(value);
        if (PyErr_Occurred()) return -1;
        if (llv < 0 || (unsigned long long)llv > UINT32_MAX) {
            PyErr_SetString(PyExc_ValueError, "max_pattern_instances must be in range [0, 4294967295]");
            return -1;
        }
        cfg->max_pattern_instances = (uint32_t)llv;
        return 0;
