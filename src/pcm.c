...
#define MEMORYPCM_PATCHES_START_AT 1024
...
void pcm_note_on(uint16_t osc) {
    if(osc_memory_patch_exists(osc)) {
        // memorypcm
        if(synth[osc].logfreq_coefs[COEF_CONST] <= 0) {
            synth[osc].logfreq_coefs[COEF_CONST] = 
                memorypcm_map[synth[osc].patch-MEMORYPCM_PATCHES_START_AT]->log2sr - 
                logfreq_for_midi_note(memorypcm_map[synth[osc].patch-MEMORYPCM_PATCHES_START_AT]->midinote);
        }
    } else {
        // const PCM
...
void pcm_note_off(uint16_t osc) {
    uint32_t length = 0;
    if(osc_memory_patch_exists(osc)) {
        length = memorypcm_map[synth[osc].patch-MEMORYPCM_PATCHES_START_AT]->length;
    } else {
...
SAMPLE render_pcm(SAMPLE* buf, uint16_t osc) {
    memorypcm_map_t patch;
    if(osc_memory_patch_exists(osc)) {
        patch = *memorypcm_map[synth[osc].patch-MEMORYPCM_PATCHES_START_AT];
    } else {
        const pcm_map_t cpatch =  pcm_map[synth[osc].patch];
...
SAMPLE compute_mod_pcm(uint16_t osc) {
    ...
    if(osc_memory_patch_exists(osc)) {
        ...
        patch.loopstart = cpatch.loopstart;
        patch.loopend = cpatch.loopend;
        patch.samplerate = PCM_AMY_SAMPLE_RATE;
    }
